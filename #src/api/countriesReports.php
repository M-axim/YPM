...
function getCountriesDataCSV($connection, $id)
{
	...
	fputcsv($output, array('Код страны', 'Наименования страны'));

	$data = $connection->query("SELECT * FROM `country` WHERE `id` = $id");
...
function getCountriesDataXLS($connection, $id)
{
...
function getCountriesEquipmentDataCSV($connection, $id)
{
	header("Content-Type: text/csv; charset=utf8");
	header("Content-Disposition: attachment; filename=patient.csv");
...
function getCountriesEquipmentDataXLS($connection, $id)
{
	header('Content-Type: application/vnd.ms-excel; charset=utf-8');
	header("Content-Disposition: attachment;filename=".date("d-m-Y")."-export.xls");
...
switch ($reportType) {
	case "CountriesData":
		if ($exportType === "export_cvs")
			getCountriesDataCSV($connection, $id);
		else if ($exportType === "export_xls")
			getCountriesDataXLS($connection, $id);
	break;
	case "CountriesEquipmentData":
		if ($exportType === "export_cvs")
			getCountriesEquipmentDataCSV($connection, $id);
		else if ($exportType === "export_xls")
			getCountriesEquipmentDataXLS($connection, $id);
	break;
}
