...
function getJobTitleDataCSV($connection, $id)
{
	...
	fputcsv($output, array('Код должности', 'Название должности', 'Зарплата'));

	$data = $connection->query("SELECT * FROM `JobTitle` WHERE `id` = $id");
...
function getJobTitleDataXLS($connection, $id)
{
...
switch ($reportType) {
	case "JobData":
		if ($exportType === "export_cvs")
			getJobTitleDataCSV($connection, $id);
		else if ($exportType === "export_xls")
			getJobTitleDataXLS($connection, $id);
	break;
	case "StaffData":
		if ($exportType === "export_cvs")
